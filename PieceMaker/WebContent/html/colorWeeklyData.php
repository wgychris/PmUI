...
$sqlSelectWeekStr = 'SELECT DISTINCT strftime(\'%W-%Y\', Date) as week FROM DailyStatistics ';
if ($startDate != null && $endDate != null) {
...
while ( $weekRow = $weekArr->fetchArray() ) {
	...
	while ( $row = $colors->fetchArray () ) {
		...
		$sql_countNum = "SELECT COUNT(OptionValue) AS colorNum FROM CustomizableOption WHERE OptionValue = '" . $colorName . "' AND strftime('%W-%Y', Date) = '" . $week . "'";
